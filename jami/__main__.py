# Python libraries
import os  # for pretty printing and memory measurement
import sys
...
Logger.info('Scanning MIDI files - this may take a moment..')
...
class Tune:
    ...
    def display(self):
        ...
        for list_index in range(len(self.harmony)):
            sys.stdout.write(
                Music.solfege[int(self.harmony[list_index][0])]+'  ')
            for note_index in range(1, len(self.harmony[list_index])-1):
                sys.stdout.write(
                    Music.solfege[int(self.harmony[list_index][note_index])]+'      ')
            sys.stdout.write(
                Music.solfege[int(self.harmony[list_index][len(self.harmony[list_index])-1])])
            print
...
def attempt(length, type):
    ...
    if type == 'random':
        ...
        for i in range(1, int(length) - 1):
            ...
            # note = Database.weightedRandom[index]
            note = Music.allPitches[random.randint(0, 11)]
...
def score(tune, explain):
    ...
    notescore = 0
    # 12 is number of possible distinct
    noteaverage = Database.oneNoteDatabase['total'] / 12.0
    numnotes = 0
    ...
    for i in range(len(tune.harmony)):
        for j in range(len(tune.harmony[i])):
            ...
            notescore += (
                Database.oneNoteDatabase[tune.harmony[i][j]]) - noteaverage
            numnotes += 1
    ...
    melscore = 0
    # 1728 is number of possible distinct
    melaverage = Database.threeNoteDatabase['total'] / 1728.0
    ...
    harmscore = 0
    harmnum = 0
    # 144 is number of possible distinct
    harmaverage = Database.harmonicDatabase['total'] / 144.0
    firstharm = ['00']
    ...
    for first in range(len(allharms)-1):  # for first 3 harmony lines
        # for every harmony line after first
        for second in range(first + 1, len(allharms)):
            for j in range(harms):  # for all harmonized positions in music
                ...
                if Database.harmonicDatabase.has_key(mume):
                    harmscore += (
                        Database.harmonicDatabase[mume]) - harmaverage
                else:
                    # pattern occures 0 times so reduce its score
...
# TODO rename
Database.store_harmonies_using_music21()
enablePrint()
...
print('Three note permutations scanned:\t',
      Database.threeNoteDatabase['total'])
print('Two note permutations scanned:\t\t', Database.twoNoteDatabase['total'])
print('Four note permutations scanned:\t\t',
      Database.fourNoteDatabase['total'])
...
while not done:
    ...
    if choice == '1':
        ...
        length = input(
            'How long would you like your tune to be? (# of notes)\n')
        try:
            num = int(num)
        # TODO
        except ValueError:
        ...
        while input != -1:
            input = input(
                'Enter note value (0-11) or \'done\' to complete melody\n')
            if input == 'done':
                ...
                if len(melody) > 2:
                    print(
                        'That mume\'s (', melody[-3:], ') score was: ', score(melody[-3:]))
        ...
        feedback = input(
            'Which melody did you like? (from among the top ten)\n')
        ...
        for i in range(len(listOfHarmonizedNotesInThisTune)):
            for first in range(len(listOfHarmonizedNotesInThisTune) - 1):
                for second in range(first + 1, len(listOfHarmonizedNotesInThisTune)):
                    harmonies.append(
                        listOfHarmonizedNotesInThisTune[first][i] + listOfHarmonizedNotesInThisTune[second][i])
